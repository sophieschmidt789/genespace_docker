...
##  Add packages needed for genespace dependencies
ADD bioc_scripts/install_orthofinder_deps.sh /tmp/

RUN bash /tmp/install_orthofinder_deps.sh

## Add host-site-library
RUN echo "R_LIBS=/usr/local/lib/R/host-site-library:\${R_LIBS}" > /usr/local/lib/R/etc/Renviron.site

## Install specific version of BiocManager
ADD bioc_scripts/install_genespace.R /tmp/
RUN R -f /tmp/install_genespace.R
